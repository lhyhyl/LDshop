          ...
          <div :class="{on:loginWay}">
            <section class="login_message">
              ...
              <button :disabled="!rightPhone" class="get_verification"
                      ...
                      {{computTime ? `已发送(${computTime}s)`  : "获取验证码"}}
              </button>
            </section>
            ...
            <section class="login_hint">
              温馨提示：未注册铃铛外卖帐号的手机号，登录时将自动注册，且代表已同意
      ...
      methods:{
        ...
        async login(){
         ...
         if(this.loginWay){//短信登录
           const {rightPhone,phone,checkCode} = this
           if(!rightPhone){
              //手机号不正确
             this.showAlert('手机号不正确')
             return
           }else if(!/^\d{6}$/.test(checkCode)){
              //短信验证码必须为6位数字
             this.showAlert('短信验证码必须为6位数字')
             return
           }
           //发送ajax请求 短信登录
            result = await reqSmsLogin(phone,checkCode)
