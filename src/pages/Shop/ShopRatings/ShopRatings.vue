    ...
    <div class="ratings-content">
      ...
      <div class="ratingselect">
        ...
        <div class="switch " :class="{on:onlyText}">
          <span class="iconfont icon-check_circle" @click="onlyText = !onlyText"></span>
          <span class="text">只看有内容的评价</span>
        </div>
      </div>
      <div class="rating-wrapper">
        <ul >
          <li class="rating-item" v-for="(shopRating,index) in filterShopRatings" :key="index">
            <div class="avatar">
              <img width="28" height="28"
                   :src="shopRating.avatar">
            </div>
            <div class="content">
              ...
              <div class="recommend" >
                ...
                <span class="item" v-for="(shopRecommend,index) in shopRating.recommend"
                      :key="index" v-show="shopRecommend">{{shopRecommend}}</span>
              </div>
              <div class="time">{{shopRating.rateTime | dataFormat}}</div>
            </div>
          </li>
        </ul>
      </div>
  ...
  import BScroll from 'better-scroll'
    export default {
      ...
      computed:{
        ...
        filterShopRatings(){
          const {shopRatings,selectType,onlyText} = this

          return shopRatings.filter((shopRating) => {
            const {rateType,text} = shopRating

            /*
            * 条件一：selectType 和 rateType
                  如果selectType === 2,就不用比较rateType了,如果不等于2，就比较
              条件二：onlyTxet为真，过滤有内容的
            * */

              return (selectType === 2 || rateType === selectType) && (!onlyText || text.length > 0)
          })
        },
      },
      components:{
